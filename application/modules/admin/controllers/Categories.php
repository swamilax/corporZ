...
class Categories extends Admin_Base_Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $crud = $this->getGroceryCRUD('categories', 'Categories', 'Manage Categories', 'Manage Categories');

            // data Grid view fields
            $crud->columns('navid', 'catname', 'link', 'flag', 'sort', 'file_path', 'created');

            // Insert form
            $crud->add_fields('navid', 'catname', 'link', 'flag', 'sort', 'file_path');

            // Update form
            $crud->edit_fields('navid', 'catname', 'link', 'flag', 'sort', 'file_path');

            //File upload
            $crud->set_field_upload('file_path', 'assets/frontend/images/categories');

            // Required fields
            $crud->required_fields('navid', 'catname', 'sort', 'file_path');

            // relationship
            $crud->set_relation('navid', 'navigation', 'catname');

            // Rename field level
            $crud->display_as('catname', 'Name');
            $crud->display_as('file_path', 'Image(w512 X h512)');
